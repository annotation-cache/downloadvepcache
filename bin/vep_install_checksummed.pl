...
$NO_CHECKSUMS ||=  $config->{NO_CHECKSUMS};
...
$CACHE_URL  ||= "https://ftp.ensembl.org/pub/release-$DATA_VERSION/variation/indexed_vep_cache";
    ...
    if(is_url($URL_TO_USE)) {
      ...
      if(!$TEST) {
        ...
        if (!$NO_CHECKSUMS) {
          ...
          die("ERROR: Could not download CHECKSUMS file from $URL_TO_USE/$checksums\n")
            unless (-e $checksums_target_file);

          my $sum_download = `sum $target_file`;
          die("ERROR: 'sum' command failed for $target_file (is the 'sum' utility installed?)\n")
            unless ($sum_download =~ m/([0-9]+)(\s+)([0-9]+)/);
          my $checksum_download = $1;
          $checksum_download =~ s/^0*//;

          my $quoted_file_name = quotemeta($file_name);
          my $sum_ftp = `grep $quoted_file_name $checksums_target_file`;
          die("ERROR: $file_name not found in CHECKSUMS file from $URL_TO_USE\n")
            unless ($sum_ftp);
          $sum_ftp =~ s/^0*//;

          die("ERROR: Checksum for $target_file doesn't match CHECKSUMS file on FTP site\n")
            if ($sum_ftp !~ m/^$checksum_download\s+/);

          print " - checksum verified for $file_name\n" unless $QUIET;
        }
      }
    }
    else {
...
--NO_CHECKSUMS     Skip checksum verification of downloaded cache files.
                   Use this if your mirror does not provide CHECKSUMS files.
